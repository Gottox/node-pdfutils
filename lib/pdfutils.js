...
function pdfutils(src, options, cb) {
	...
	if(typeof src === 'string' || src instanceof String) {
		...
		native.openFromPath(doc, pdfPageFactory, options.password, src,
							   cbWrapper(cb, doc));
	}
	else if(Buffer.isBuffer(src)) {
		// Async Read Buffer
		if(cb) {
			reader = native.openFromReader(doc, PdfPage, options.password,
					cbWrapper(cb, doc));
			reader.data(src);
			reader.end();
		}
		// Sync Read Buffer
			...
			native.openFromData(doc, pdfPageFactory, options.password, src);
		}
	}
